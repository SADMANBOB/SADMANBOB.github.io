...
const processSteps = [
  ["Submit the request", contractorUploadPolicy
    ? "Share the address, occupancy, requested work, timing, inspection-eligibility answer, and only authorized files through the approved protected path."
    : "Share the address, occupancy, requested work, timing, and inspection-eligibility answer. Photos must wait for an approved sharing path."],
...
function Home({ onNavigate }) {
  ...
  return <>
    ...
    <section className="hero"><div className="container hero-grid"><div className="hero-copy"><span className="eyebrow">C&amp;G Contracting Services</span><h1>Practical repairs. <em>Built to last.</em></h1><p>Residential repair and improvement services across Los Angeles County and Riverside County, subject to project review. Start with a clear description of the property, the condition, and the result you want. C&amp;G reviews eligibility, access, trades, permits, materials, schedule, and license fit before offering an estimate or accepting work.</p><div className="hero-actions"><InternalLink className="button button-copper" href="/estimate/" onNavigate={onNavigate}>Request an Estimate <ArrowRight size={16} aria-hidden="true" /></InternalLink><InternalLink className="text-link text-link-dark" href="/services/" onNavigate={onNavigate}>Explore Services <ArrowRight size={15} aria-hidden="true" /></InternalLink></div></div><figure className="hero-image"><PublishedWorkPicture editorialId="contractor-hero" surface={PUBLICATION_SURFACES.contractorHomePhoto} sizes="(max-width: 900px) 100vw, 52vw" fetchPriority="high" /><figcaption>{heroPhoto ? heroPhoto.caption : "Representative editorial image · not client project photography"}</figcaption></figure></div></section>
    ...
    {showProvisional ? <section className="services-section typical-projects-section" aria-labelledby="typical-projects-title"><div className="container"><div className="section-heading split-heading"><div><span>Typical project categories</span><h2 id="typical-projects-title">Work types often discussed in the first review.</h2></div><p><span className="provisional-label">Provisional · owner review</span> {typicalProjects.qualification}</p></div><ul className="typical-projects-grid">{typicalProjects.categories.map((category) => <li key={category}>{category}</li>)}</ul><p className="provisional-note">These categories are provisional_owner_review content for staging completeness. They do not accept work, replace license review, or promise that every listed item fits a specific property.</p><InternalLink className="section-link" href="/services/" onNavigate={onNavigate}>See detailed categories and boundaries <ArrowRight size={15} aria-hidden="true" /></InternalLink></div></section> : null}
...
function ServiceArea({ route, onNavigate }) {
  ...
  return <>
    ...
    <section className="about-story"><div className="container about-story-grid"><div><span>Property context</span><h2>Scope begins with the actual address.</h2><p>{area.pageContent.propertyContext}</p></div><div className="about-story-copy"><span>Access and timing</span><h2>Confirm the practical constraints before relying on a work date.</h2><p>{area.pageContent.accessAndTiming}</p><p>{area.qualification || serviceAreaQualification}</p></div></div></section>
