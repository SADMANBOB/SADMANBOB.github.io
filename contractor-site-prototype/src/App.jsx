...
const contractorFileRequest = externalFileRequestActionFor("contractor-estimate");
...
const processSteps = [
  ["Tell us what needs attention", contractorUploadPolicy
    ...
    : contractorFileRequest
      ? `Share the address, occupancy, requested work, timing, and the 12-month inspection-eligibility answer. After the request is received, you may send up to ${contractorFileRequest.requestedMaxFiles} optional project photos through the separate approved file request.`
      : "Share the address, occupancy, requested work, timing, and the 12-month inspection-eligibility answer. Do not send photos with the initial request. If C&G asks for them later, follow the separate sharing instructions provided."],
...
function ServiceArea({ route, onNavigate }) {
  ...
  return <>
    ...
    <section className="values-section"><div className="container"><h2>Prepare the first project conversation</h2><ul>{area.pageContent.planningChecklist.map((item) => <li key={item}><Check aria-hidden="true" />{item}</li>)}</ul><div className="home-area-card-actions"><InternalLink className="button button-graphite" href="/estimate/" onNavigate={onNavigate}>Review eligibility and request details</InternalLink><a className="button button-outline-dark" href="/inspection/">Home inspection sister site</a></div></div></section>
    <SeparationNotice onNavigate={onNavigate} />
  </>;
}
