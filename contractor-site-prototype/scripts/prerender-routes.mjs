...
import { enabledContractorRoutes, contractorNotFoundRoute } from "../src/content/routes.js";
import { buildContractorSchema } from "../src/content/seo.js";
...
const contractorOrigin = `${siteOrigin}/contracting`;
...
const setHead = (shell, route, path, body, { noindex = false } = {}) => {
  const url = `${contractorOrigin}${path === "/" ? "/" : path}`;
  const imageUrl = `${contractorOrigin}/assets/contractor-hero.jpg`;
  ...
  const schema = JSON.stringify(buildContractorSchema({ ...route, path }, contractorOrigin)).replaceAll("<", "\\u003c");
  html = html.replace("</head>", `    <script id="cg-page-schema" type="application/ld+json">${schema}</script>\n  </head>`);
  return html.replace('<div id="root"></div>', `<div id="root">${body}</div>`);
};

const shell = await readFile(resolve(dist, "index.html"), "utf8");
for (const route of enabledContractorRoutes) {
...
await writeFile(resolve(dist, "404.html"), setHead(shell, contractorNotFoundRoute, "/404.html", notFoundBody, { noindex: true }));
await Promise.all(["contractor-hero.png", "finish-work.png", "project-planning.png"].map((file) => rm(resolve(dist, "assets", file), { force: true })));
